...
class MovimentacaoConjuntoBem {

    static function bundle() {
        $u = new MovimentacaoConjuntoBem();
        return $u;
    }

    function start_pdf($patrimonios) {
        ...
        <p><b>UNIVERSIDADE FEDERAL DO CEARÁ<br />CAMPUS DE CRATEÚS<br />SISTEMA DE ALMOXARIFADO E PATRIMÔNIO - SIAP</b><br />
            EMITIDO EM ' . $data . ' ' . $hora . '</p>'
                . '</div><br />';
        $titulo = '<div style="text-align:center;"><p><b>RELATÓRIO MOVIMENTAÇÕES DOS BENS</b></p></div><br />';
        $tabel = '';
        foreach ($patrimonios as $tombamento) {
            $movimentacoes = Movimentacao::getAllByPatrimonioEntradaOrderById($tombamento);
            ...
            $tabel .= '<div class="container" style="border:2px solid #f0f0f0; border-radius:10px;font-family:sans-serif;">'
                    ...
                    . '<div class="panel-heading" style="width:100%;display:block;background:#f0f0f0;padding:5px 10px;">' . Ativos::getById($tombamento)->getNome() . '</div>'
